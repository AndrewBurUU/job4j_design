...
select c.id, c.name car_name, 
cb.name body_name, ce.name engine_name, cm.name transmission_name
from cars c
left join carbodies cb on c.body_id = cb.id
left join carengines ce on c.engine_id = ce.id
left join cartransmissions cm on c.transmission_id = cm.id;
